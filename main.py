...
def get_recipes(food, diet):
    try:
        ...
        response = model.generate_content(
            f"Suggest 3 yummy {diet} recipes using {food} as the main ingredient. Use emojis and keep it short.")
...
with st.sidebar:
    ...
    st.markdown("### 🧭 Menu")
    page = st.radio("Go to:", [
                    "🏠 Home & Health", "📸 Scan & Eat", "📊 My Diary"], label_visibility="collapsed")
    ...
    with st.expander("👤 Edit Profile", expanded=False):
        ...
        if st.button("Save Stats"):
            ...
            st.success(
                f"Updated! Goal: {st.session_state['daily_goal']} kcal 🚀")
...
if "Home" in st.session_state['page']:
    st.markdown("<h1 style='text-align: center; color: #00E676;'>🥗 NutriScan AI</h1>",
                unsafe_allow_html=True)
...
# --- PAGE: SCANNER ---
    ...
    with c1:
        st.markdown("### 📸 Input")
        src = st.radio("Source", ["Upload 📁", "Camera 📷"],
                       horizontal=True, label_visibility="collapsed")
        img_file = st.file_uploader(
            "File", type=['jpg', 'png']) if "Upload" in src else st.camera_input("Snap")
            ...
            if st.button("✨ Generate Recipes"):
                with st.spinner("Cooking up ideas..."):
                    st.session_state['recipe_result'] = get_recipes(
                        st.session_state['scan_data']['name'], st.session_state['user_diet'])
    ...
    with c2:
        if st.session_state['scan_data']:
            ...
            fig.update_layout(height=250, margin=dict(
                t=0, b=0, l=0, r=0), paper_bgcolor='rgba(0,0,0,0)')
...
# --- PAGE: DIARY ---
    ...
    if st.session_state['food_log']:
        ...
        st.metric("Total Calories", f"{total}",
